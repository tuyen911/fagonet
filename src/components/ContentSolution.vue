<template>
  <div>
    <section class="content-solution">
            <div class="home-solutions text-black">
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">SIEM - Security Information and Event Management</h4>
                    <a class="text-end " href="/solutions/siem">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">WAF - Web Application Firewall</h4>
                    <a class="text-end " href="/solutions/waf">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">NIPS - Network Intrusion Prevention System</h4>
                    <a class="text-end " href="/solutions/nips">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">EDR - Endpoint Detection and Response</h4>
                    <a class="text-end " href="/solutions/edr">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">NAC - Network Access Control</h4>
                    <a class="text-end " href="/solutions/nac">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                    <div class="lazyload-wrapper ">
                        <img class="Scale" src="data:image/png;base64," alt="vector-img">
                    </div>
                    <h4 class="text-start">NIDS - Network Intrusion Detection System</h4>
                    <a class="text-end " href="/solutions/nids">
                        <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                    </a>
                </article>
                <article class="relative notHoverSolutions">
                        <div class="lazyload-wrapper ">
                            <img class="Scale" src="data:image/png;base64," alt="vector-img">
                        </div>
                        <h4 class="text-start">DIP - Detailed Analysis Of Data Packets Passing Through The Network</h4>
                        <a class="text-end " href="/solutions/dip">
                            <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                        </a>
                </article>
                <article class="relative notHoverSolutions">
                        <div class="lazyload-wrapper ">
                            <img class="Scale" src="data:image/png;base64," alt="vector-img">
                        </div>
                        <h4 class="text-start">DIP - Detailed Analysis Of Data Packets Passing Through The Network</h4>
                        <a class="text-end " href="/solutions/dip">
                            <i class="fa fa-long-arrow-right text-3xl Scale"></i>
                        </a>
                </article>
            </div>
            <div class="solution">
                <h1 class="text2">SOLUTION</h1>
                <p class="my-text">We Provide Cyber Security Solutions That Meet The Unique Needs Of Our Customers And Help Them Succeed In An Increasingly Digital World.</p>
                <div class="lazyload-wapper">
                <img src="https://fagonet.com/assets/Thumbsup1-2e610cd9.png">
                </div>
            </div>
    </section>
  </div>
</template>

<script>
export default {
name: 'contentsolution'
}
</script>

<style scoped>
.content-solution{
    display:flex;
    align-items: center;
    justify-content: space-between;
    margin: 0 146px;
}
.my-text{
    font-size:20px;
    line-height:30px;
    margin:16px 0;
    text-align: center;
    color:#f5f5f5;
}
.home-solutions{
    display: grid;
    grid-template-columns: repeat(2,1fr);
    gap: 24px;  
    padding:80px 0 ;
    
}
.home-solutions>article {
    height: 210px;
    cursor: pointer;
    background: linear-gradient(180deg,rgba(255,255,255,.3) 0%,rgba(255,255,255,.1) 100%);
    -webkit-backdrop-filter: blur(5px);
    backdrop-filter: blur(5px);
    font-size: 20px;
    padding: 20px;
    border-radius: 25px;
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    color: #fff;
    position:relative;
}
.home-solutions>article:hover{
    transform: scale(.98);
    color:#003249 !important;
    background:#fff;
}
.lazyload-wrapper{
    text-align: start;
}
.text-start{
    text-align: start;
    font-size:18px;
    font-weight:700;
    margin-top: .5rem;
    margin-bottom: .5rem;
    color:#fff;
}
.text-black{
    --tw-text-opacity: 1;
    color: rgb(0 9 19/var(--tw-text-opacity));
}
.text-end{
    position:absolute;
    right:2rem;
    top:.5rem;
    display:block;
    text-align: end;
    font-weight:700;
    --tw-text-opacity: 1;
    color: rgb(33 90 116/var(--tw-text-opacity));
}
</style>